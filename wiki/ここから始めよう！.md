...
## MikuMikuWorld 譜面プレビュー

<img width="128" height="128" alt="mmw_icon" src="https://github.com/user-attachments/assets/87d87e73-d810-4267-a0c1-4787416c8241" />

0. 譜面プレビューに対応したMikuMikuWorldを起動し、譜面ファイルをインポートします
  - 例：[monti114514/MikuMikuWorld4UC-by-Monchi](https://github.com/monti114514/MikuMikuWorld4UC-by-Monchi) and [watagashi-uni/External-Unofficial-ChartMaker](https://github.com/watagashi-uni/External-Unofficial-ChartMaker)

> [!NOTE]
> **譜面メーカー**で作成した譜面の場合、譜面ファイルが生成され、`pjsekai-overlay-APPEND\dist\[譜面ID]`（`.pjsk.json` および `.usc`）に保存されますので、そこからインポートしてください（二次利用が禁止されているものを除く）。ご自由に利用してください。

> [!WARNING]
> `--force-derivative`フラグを使用すると、二次利用を禁止した譜面メーカーの譜面ファイルを強制的に保存することができます。ただし、`--force-derivative`を使用するのは、二次利用の許可がNGになっている自身の譜面を使用する場合に限ってください。**明示的な許可がない限り、他の作成者の譜面に対してこのフラグを使用しないでください！！**

1. **(重要)** 譜面がすぐに始まる場合、すべてのノーツ、ハイスピード、BPM、拍子を（使用するノーツの速さに応じて）少なくとも1～3秒遅らせる必要があります
2. `設定 (Ctrl + ,) → プレビュー` へ移動し、ノーツ以外のすべての視覚効果をオフにしてください（例：`プレビューツールバーを描く`、`ステージの不透明度`、`背景画像を描く` など）。
3. `F5`キーを押して、プレビュー画面を全画面表示に切り替えます
4. 画面を録画します（推奨：[OBS](https://obsproject.com/) または `Win + Alt + R` のキーバインド）
...
7. exo/alias(.object)が提供する**_タイムライン_で動画と音声ファイルを選択**

> [!WARNING]
> この手順では、_timeline_内の動画要素に[Unmult](https://github.com/mes51/AVIUtl2_Unmult)を使用します。これにより、すべての黒ピクセルが透明ピクセルに変換され、正確なMV効果が得られます。**これは「合成モード」を選択することとは異なり、同じ効果は得られません！！**
...
| **キャッシュ**<br>`cache`    	| データを`data.ped`にキャッシュします。キャッシュが0に設定されている場合、`data.ped`の変更は即時反映されます<br>(`0` - OFF, `1` - ON)                               	|    1    	|        0か1      	|
...
| **名前**         | 説明                                                                                                    | デフォルト     |        範囲           |
|----------	|-------------------------------------------------------------------------------------	|:-------:	|:--------:	|
| **ライフ**<br>`life` 	| LIFEの値（自明）<br>- **(ロック中)** LIFEの値は、LIFEのスキルイベントに基づいて自動的に調整されます<br>- **(ロック解除)** 値が変化すると、LIFEバーもそれに応じて変化します 	|   1000  	| 0 ~ 9999 (整数) 	|
| **スキル効果**<br>`life_skill`     | スキル発光効果の切り替え<br>`0`: OFF<br>`1`: AUTO - 各スキル発動時、5秒間スキル発光効果を表示<br>`2`: ON       |    1      | 0 ～ 2 (整数)    |
...
| **ライフバーアンロック**<br>`unlock_life`     | LIFE値を自由に調整する     |  0 (OFF)      |    0か1       |
...
| **名前**         | 説明                                                                                                    | デフォルト     |        範囲           |
|-----------------------------------	|----------------------------------------	  |:-------:|:-----:	|
| **アニメーション速度**<br>`skill_speed`                     	| アニメーション速度を調整する 	|    1.00    	| 0以上	|
| **キャッシュのアイコン**<br>`skill_cache`                     	| **(AviUtl ExEdit2のみ)** カードのアイコンを `\assets\skill\profile` フォルダにキャッシュする             	  |    1   	| 0か1 	|
...
### AviUtlでのカードアイコンの変更
新しいカードアイコンを取得するには、**`pjsekai-overlay-APPEND\assets\skill\profile`** ディレクトリ内のREADMEに従って、公式ゲームからアイコンを取得してください。**新しいアイコンはこのフォルダに配置します。**
**AviUtl ExEdit2**では、新しいアイコンを配置するとアイコン一覧が自動的に更新されるため、そのフォルダにアイコンを配置するだけで済みます。一方、**AviUtl**ではアイコン一覧は更新されないため、手動で追加する必要があります。

- `スキル@pjsekai-overlay > 設定 > アイコンのファイル名` に移動し、次のように1つまたは複数のファイル名を入力してください：`image1.png,image2.png,image3.png,...`。

<img width="50%" height="auto" alt="image" src="https://github.com/user-attachments/assets/af031026-654b-4cf2-86fe-0b6e181317a8" />
